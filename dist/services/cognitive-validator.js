 ...
 */
export function validateCognitiveIntegration(parsed, strict = false) {
    const errors = [];
    const warnings = [];
 ...
 */
export function generateDefaultCognitiveIntegration(parsed) {
    const description = parsed.frontmatter.description.toLowerCase();
    const steps = parsed.body.steps?.join(' ').toLowerCase() || '';
    const overview = parsed.body.overview?.toLowerCase() || '';
    const content = `${description} ${steps} ${overview}`;
 ...
 */
export function hasSufficientCognitiveIntegration(parsed) {
    const ci = parsed.frontmatter.cognitive_integration;
    const missing = [];
    if (!ci) {
...
//# sourceMappingURL=cognitive-validator.js.map