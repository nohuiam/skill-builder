 ...
 */
export function countLayer1Tokens(name, description) {
 ...
 */
export function countLayer2Tokens(fullContent) {
    return countTokens(fullContent);
}
/**
 * Check if content fits within progressive disclosure limits
 */
export function checkProgressiveDisclosure(layer1Tokens, layer2Tokens) {
    const warnings = [];
    const layer1Ok = layer1Tokens <= SKILL_CONFIG.LAYER1_MAX_TOKENS;
    const layer2Ok = layer2Tokens <= SKILL_CONFIG.LAYER2_MAX_TOKENS;
    if (!layer1Ok) {
        warnings.push(`Layer 1 (metadata) exceeds limit: ${layer1Tokens} tokens > ${SKILL_CONFIG.LAYER1_MAX_TOKENS} max`);
    }
    if (!layer2Ok) {
        warnings.push(`Layer 2 (full content) exceeds limit: ${layer2Tokens} tokens > ${SKILL_CONFIG.LAYER2_MAX_TOKENS} max`);
    }
    return {
 ...
 */
export function getTokenBreakdown(content, name, description) {
