...
export class DatabaseManager {
    ...
    // Skill Queries
    // =============================================================================
    getAllSkills(includeDeprecated = false) {
    ...
    getSkillMetadata(includeDeprecated = false) {
        const query = includeDeprecated
    ...
    searchSkills(search, includeDeprecated = false) {
    ...
    getSkillsByTags(tags, includeDeprecated = false) {
    ...
    insertUsage(usage) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, usage.skill_id, usage.outcome, usage.context ? JSON.stringify(usage.context) : null, usage.notes || null, usage.duration_ms || null, usage.created_at || Date.now());
    ...
    getSkillUsages(skillId, limit = 100) {
        const stmt = this.db.prepare(`
      ...
      LIMIT ?
    `);
        const rows = stmt.all(skillId, limit);
        return rows.map(row => ({
            id: row.id,
            skill_id: row.skill_id,
            outcome: row.outcome,
    ...
    getSkillSuccessRate(skillId) {
        const skill = this.getSkill(skillId);
        if (!skill || skill.usage_count === 0)
            return 0;
    ...
    insertVersion(version) {
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
        stmt.run(version.id, version.skill_id, version.version, version.content, version.created_at);
        return version.id;
    }
    ...
    `);
        const row = stmt.get(skillId, version);
        if (!row)
            return null;
        return {
            id: row.id,
    ...
    // =============================================================================
    getStats() {
        const skills = this.db.prepare('SELECT COUNT(*) as count FROM skills').get();
        const active = this.db.prepare('SELECT COUNT(*) as count FROM skills WHERE deprecated_at IS NULL').get();
        const deprecated = this.db.prepare('SELECT COUNT(*) as count FROM skills WHERE deprecated_at IS NOT NULL').get();
        const usages = this.db.prepare('SELECT COUNT(*) as count FROM skill_usages').get();
