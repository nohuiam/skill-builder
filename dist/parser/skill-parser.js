 ...
 */
function extractSection(body, headingVariants) {
    // Create regex pattern for section headings
    const headingPatterns = headingVariants.map(h => `^#+\\s*${escapeRegex(h)}\\s*$`).join('|');
    const sectionRegex = new RegExp(`(${headingPatterns})\\s*\\n([\\s\\S]*?)(?=^#+\\s|$)`, 'gmi');
 ...
 */
export function generateSkillMd(frontmatter, body) {
    const lines = [];
